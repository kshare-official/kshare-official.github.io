...
function setupTableOfContents() {
    ...
    if (tocHeading) {
        ...
        // Accessibility: Wrap content in a native button instead of changing h2 role
        const text = tocHeading.innerHTML;
        tocHeading.innerHTML = '';
        const btn = document.createElement('button');
        btn.innerHTML = text;
        btn.setAttribute('aria-expanded', tocHeading.classList.contains('expanded') ? 'true' : 'false');
        
        // Reset button styles so it blends seamlessly into the h2
        btn.style.all = 'unset';
        btn.style.cursor = 'pointer';
        btn.style.flexGrow = '1';
        btn.style.textAlign = 'left';
        
        tocHeading.appendChild(btn);
